...
// csiClusterMappingRecord represent the structure to serialize a csi mapping
// record in Ceph CSI's config, configmap
...
func (r *ClientProfileMappingReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		// TODO: This watch is probalmatic, it will trigger a reconcile for each
...
func (r *ClientProfileMappingReconcile) reconcile() error {
	...
	// building it from the gournd up on every reconcile. To do that we ignore the specific of the subject resource
	...
	_, err := ctrlutil.CreateOrUpdate(r.ctx, r.Client, &csiConfigMap, func() error {
		...
		for i := range r.clientProfileMappingList.Items {
			...
			for j := range spec.Mappings {
				...
				// Check if we already encountered the local+remote pair. If we did, add
				// the current poolId mapping information to the existing record. if not,
				// append a new record at the end.
				if index, ok := indexByPair[key]; ok {
					maps.Copy(
						csiClusterMappingsList[index].RbdPoolIdMapping[0],
						mapping.BlockPoolIdMapping,
					)
				} else {
					...
					csiClusterMappingsList = append(
						...
						csiClusterMappingRecord{
							...
							RbdPoolIdMapping: []map[string]string{
								maps.Clone(mapping.BlockPoolIdMapping),
							},
						},
					)
				}
